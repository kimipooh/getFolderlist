...
function onOpen(e){
    var arr = [
        {name: "リスト作成", functionName: "getFolderlist"}
    ];
...
function listAllFandF() {
  ...
  sheet.getRange(j, 2).setValue("ファイル名");
  sheet.getRange(j, 3).setValue("作成日");
  sheet.getRange(j, 4).setValue("最終更新日");
  sheet.getRange(j, 5).setValue("オーナー");
  sheet.getRange(j, 6).setValue("URL");
  j++;
  // ボディ記入
  for (i=0; i<=folderlist.length-1; i++) {
    ...
    while(files.hasNext()){
      ...
      sheet.getRange(j, 4).setValue(file.getLastUpdated());
      if(file.getOwner()){
        sheet.getRange(j, 5).setValue(file.getOwner().getName());
      }else{ // チームドライブはオーナーなし
        sheet.getRange(j, 5).setValue("チームドライブ");
      }
      sheet.getRange(j, 6).setValue(file.getUrl());
      j++;
    }
  } 
}